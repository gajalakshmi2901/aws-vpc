# Create VPC based on the value true or false
...
#IPv4 association - aws_vpc_ipv4_cidr_block_association resource is used to associate additional IPv4 CIDR blocks with an existing Virtual Private Cloud (VPC)
resource "aws_vpc_ipv4_cidr_block_association" "ipv4" {
...
#   )
# }
# #Public  Rouble Table associated with public subnets
# resource "aws_route_table_association" "publicrt" {
#   count = var.create_pub_association_default_rt && local.create_public_subnets ? local.len_public_subnets : 0
#   subnet_id      = element(aws_subnet.public[*].id, count.index)
#   route_table_id = aws_route_table.public[0].id
# }
# #Below code creates route for IGW to internet 
...
#   count = var.create_pri_association_default_rt && local.create_private_subnets ? local.len_private_subnets : 0
...
resource "aws_network_acl" "private" {
  count = var.create_private_network_acl ? 1 : 0
...
resource "aws_network_acl_rule" "private_inbound" {
  count = var.create_private_network_acl ? length(var.private_inbound_acl_rules) : 0
...
resource "aws_network_acl_rule" "private_outbound" {
  count = var.create_private_network_acl ? length(var.private_outbound_acl_rules) : 0
...
#   count = var.create_vpc && var.create_egress_only_igw && var.enable_ipv6 ? local.len_private_subnets : 0
...
#Create Route for NAT. Via this route private subnet instances will communicate to internet/other CIDR
...
#   count = var.create_vpc && var.propagate_private_route_tables_vgw && (var.enable_vpn_gateway || var.vpn_gateway_id != "") ? local.len_private_subnets : 0
...
resource "aws_cloudwatch_log_group" "flow_log_cloudwatch_loggroup" {
...
resource "aws_iam_role" "iam_vpc_flow_log_cloudwatch" {
...
resource "aws_iam_role_policy_attachment" "vpc_flow_log_cloudwatch" {
  ...
  role       = aws_iam_role.iam_vpc_flow_log_cloudwatch[0].name
